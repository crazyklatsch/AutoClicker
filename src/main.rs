    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                add_loop_action(ui, 0, &mut self.root_action);
            });
        });
...
fn add_loop_action(ui: &mut Ui, depth: u16, loopaction: &mut LoopAction) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.vertical(|ui| {
            for (pos, action) in loopaction.actions.iter_mut().enumerate() {
                ui.group(|ui| {
                    ui.horizontal(|ui| {
                        ui.label(format!("{}.{}: ", depth, pos));
                        match action {
                            Action::Loop(val) => {
                                add_loop_action(ui, depth + 1, val);
                            }
                            Action::Move(val) => {
                                ui.label("x: ");
                            ...
                            Action::Press(val) => {
                                egui::ComboBox::new(pos + usize::from(depth) * 1000, "Key")
